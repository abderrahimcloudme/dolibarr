...
class User extends CommonObject
{
     ...
     * @return void
	 */
	function containing($id, $mode='object')
	{
		$users = array();
		$sql = "SELECT ct.fk_user, c.lastname, c.rowid";
		$sql .= " FROM " . MAIN_DB_PREFIX . "user_project" ." as ct, " . MAIN_DB_PREFIX . "user as c";
		$sql .= " WHERE ct.fk_user = c.rowid AND ct.fk_project = " . (int) $id ;
...
function add_projet($obj)
	{
		...
		$sql = "INSERT INTO " . MAIN_DB_PREFIX . "user_project" ;
		$sql .= " (fk_user, fk_project)";
		$sql .= " VALUES (" . $this->id . ", " . $obj->id . ")";

		dol_syslog(get_class($this).'::add_projet', LOG_DEBUG);
	...
	function del_projet($obj)
	{
		...
		$sql = "DELETE FROM " . MAIN_DB_PREFIX . "user_project" ;
		$sql .= " WHERE fk_user = " . $this->id;
		$sql .= " AND   fk_project  = " . $obj->id;

		dol_syslog(get_class($this).'::del_projet', LOG_DEBUG);
		if ($this->db->query($sql))
		{
            // Call trigger
			...
			//$result=$this->call_trigger('USER_UNLINK',$user);
            //if ($result < 0) { $error++; }
