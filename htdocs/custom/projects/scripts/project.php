...
/**
 *      \file       scripts/project.php
 *		\ingroup    projects
 *      \brief      This file is an example for a command line script to work on Project
 */
...
// Examples for manipulating class Project
...
$project->prop1='value_prop1';
$project->prop2='value_prop2';
$id=$project->create($user);
if ($id < 0) { $error++; dol_print_error($db,$project->error); }
...
if ($result < 0) { $error; dol_print_error($db,$project->error); }
else print "Object with id=".$id." loaded\n";
*/

// Example for updating object in database ($project must have been loaded by a fetch before)
/*
dol_syslog($script_file." UPDATE", LOG_DEBUG);
$project->prop1='newvalue_prop1';
$project->prop2='newvalue_prop2';
$result=$project->update($user);
if ($result < 0) { $error++; dol_print_error($db,$project->error); }
else print "Object with id ".$project->id." updated\n";
*/

// Example for deleting object in database ($project must have been loaded by a fetch before)
/*
dol_syslog($script_file." DELETE", LOG_DEBUG);
$result=$project->delete($user);
if ($result < 0) { $error++; dol_print_error($db,$project->error); }
else print "Object with id ".$project->id." deleted\n";
*/
