...
/**
 * \file    class/api_gestioncommerciaux.class.php
 * \ingroup gestioncommerciaux
 * \brief   File for API management of contact.
 */

/**
 * API class for gestioncommerciaux contact
 *
 * @smart-auto-routing false
 * @access protected
 * @class  DolibarrApiAccess {@requires user,external}
 */
class GestionCommerciauxApi extends DolibarrApi
{
    /**
    ...
    function get($id)
    {
		if(! DolibarrApiAccess::$user->rights->contact->read) {
    ...
    function index($sortfield = "t.rowid", $sortorder = 'ASC', $limit = 100, $page = 0, $sqlfilters = '')
    {
        ...
        $sql.= " FROM ".MAIN_DB_PREFIX."contact_mytable as t";
        ...
        if ($tmpobject->ismultientitymanaged) $sql.= ' AND t.entity IN ('.getEntity('contact').')';
        ...
        if ($result)
        {
            ...
            throw new RestException(503, 'Error when retrieve contact list');
        }
        if( ! count($obj_ret)) {
            throw new RestException(404, 'No contact found');
        }
		return $obj_ret;
    }
    ...
    {
        if(! DolibarrApiAccess::$user->rights->contact->create) {
            throw new RestException(401);
        }
        ...
        if( ! $this->contact->create(DolibarrApiAccess::$user)) {
    ...
    function put($id, $request_data = null)
    {
        ...
        if($this->contact->update($id, DolibarrApiAccess::$user))
            return $this->get($id);
    ...
    function delete($id)
    {
    	if(! DolibarrApiAccess::$user->rights->contact->delete) {
			throw new RestException(401);
		}
        $result = $this->contact->fetch($id);
        if( ! $result ) {
