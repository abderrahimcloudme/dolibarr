...
if (empty($reshook))
{
		...
		else
		{
			...
			if (!$error)
			{
				...
				$listofproperties=array(
					'address', 'zip', 'town', 'state_id', 'country_id', 'phone', 'origin','ccp','tccp', 'phone_pro', 'fax', 'email', 'skype', 'twitter', 'facebook', 'url', 'barcode',
    ...
    && ($action == 'add' || $action == 'update') && $user->rights->societe->creer)
    {
        ...
        if (! $error)
        {
            ...
            $object->ccp					= GETPOST('ccp','alpha');
	        $object->tccp					= GETPOST('tccp','alpha');
...
$Rightcommercial = new RightsGestionDesCommerciaux($db);
    ...
    $isCommercial = $Rightcommercial->checkIfCommercial($user->id);
    if ($isCommercial != 1){
    	print ' 
    	<script type="text/javascript">
    	$(document).ready(function(){
        $("#formsoc :input").prop("disabled", true);
   			});
		</script>';
    }
...
else
{
    ...
    if ($action == 'create')
    {
        ...
        if (! empty($conf->use_javascript_ajax))
        {
	        ...
	        print '</label>';
            print ' &nbsp; &nbsp; ';
	        print '<label for="radioprivate" style ="display:none!important;"  class="radioprivate">';
            $text ='<input style ="display:none!important;" type="radio" id="radioprivate" class="flat" name="private" value="1"'.($private?' checked':'').'>';
	        $text.='&nbsp;';
        ...
        print '<form enctype="multipart/form-data" action="'.$_SERVER["PHP_SELF"].'" method="post" name="formsoc" autocomplete="off" id="formsoc">';		// Chrome ignor autocomplete
        ...
        print '<td style="display:none!important;">'.fieldLabel('CustomerCode','customer_code').'</td><td>';
        print '<table class="nobordernopadding"><tr><td>';
		$tmpcode=$object->code_client;
        if (empty($tmpcode) && ! empty($modCodeClient->code_auto)) $tmpcode=$modCodeClient->getNextValue($object,0);
        print '<input style="display:none!important;" type="text" name="customer_code" id="customer_code" class="maxwidthonsmartphone" value="'.dol_escape_htmltag($tmpcode).'" maxlength="15">';
        print '</td><td style="display:none!important">';
        ...
        if (empty($conf->global->SOCIETE_DISABLE_STATE))
        {
            ...
            {
                print '<tr style="display:none!important;"><td>'.fieldLabel('Region-State','state_id').'</td><td colspan="3" class="maxwidthonsmartphone">';
            }
            else
            {
                print '<tr style="display:none!important;"><td>'.fieldLabel('State','state_id').'</td><td colspan="3" class="maxwidthonsmartphone">';
            }
        ...
        print '<tr style="display:none!important;"><td>'.fieldLabel('Web','url').'</td>';
        ...
        print '<td style="display:none!important;">'.fieldLabel('Fax','fax').'</td>';
	    print '<td style="display:none!important;"><input type="text" name="fax" id="fax" class="maxwidth100onsmartphone quatrevingtpercent" value="'.$object->fax.'"></td></tr>';
	    print "\n".'<script type="text/javascript">';
            print '
						function showccp(that) {
    						
    						if (that.value == "cEntrant") {
        					document.getElementById("canalconnaissanceprojet1").style.display = "table-row";
        					document.getElementById("contactentrant").classList.remove("hidden");
        					document.getElementById("ccpce").setAttribute("name", "ccp"); 
        						
    						} else {
        					document.getElementById("canalconnaissanceprojet1").style.display = "none";
        					document.getElementById("contactentrant").classList.add("hidden");
    						}

    						if (that.value == "cDigital") {
        					document.getElementById("canalconnaissanceprojet2").style.display = "table-row";
        					document.getElementById("contactdigital").classList.remove("hidden");
        					document.getElementById("ccpcd").setAttribute("name", "ccp"); 
        						
    						} else {
        					document.getElementById("canalconnaissanceprojet2").style.display = "none";
        					document.getElementById("contactdigital").classList.add("hidden");
    						}
    						if (that.value == "cEvenement") {
        					document.getElementById("labelnomevenement").style.display = "table-row";
        					document.getElementById("nomevenement").classList.remove("hidden");
        					document.getElementById("nomevenements").setAttribute("name", "tccp");
        					document.getElementById("nomprojets").setAttribute("name", "tccp1");
        						
    						} else {
        					document.getElementById("labelnomevenement").style.display = "none";
        					document.getElementById("nomevenement").classList.add("hidden");
    						}
    						if (that.value == "cProjet") {
        					document.getElementById("labelnomprojet").style.display = "table-row";
        					document.getElementById("nomprojet").classList.remove("hidden");
        					document.getElementById("nomprojets").setAttribute("name", "tccp");
        					document.getElementById("nomevenements").setAttribute("name", "tccp1");
        						
    						} else {
        					document.getElementById("labelnomprojet").style.display = "none";
        					document.getElementById("nomprojet").classList.add("hidden");
    						}

						}
						
						function showtccp(that) {
						if (that.value == "AppelEntrant") {
        						document.getElementById("labeloriginepub").classList.remove("hidden");
        						document.getElementById("originepub").classList.remove("hidden");
        						document.getElementById("originePub").setAttribute("name", "tccp"); 

    							} else {
        						document.getElementById("labeloriginepub").classList.add("hidden");
        						document.getElementById("originepub").classList.add("hidden");
    							}
    					
    					if (that.value == "Parrainage") {
        						document.getElementById("labelparrain").classList.remove("hidden");
        						document.getElementById("parrain").classList.remove("hidden");
        						document.getElementById("parrainage").name= "tccp"; 

    							} else {
        						document.getElementById("labelparrain").classList.add("hidden");
        						document.getElementById("parrain").classList.add("hidden");
    							}
    					
    					if (that.value == "Mandataire") {
        						document.getElementById("labelmandataire").classList.remove("hidden");
        						document.getElementById("mandataire").classList.remove("hidden");
        						document.getElementById("mandataire").name= "tccp"; 

    							} else {
        						document.getElementById("labelmandataire").classList.add("hidden");
        						document.getElementById("mandataire").classList.add("hidden");
    							}
	    ...
	    print '<tr><td>'.fieldLabel('Source','origin').'</td>';
	    print '<td><select name="origin" id="source" onchange="showccp(this);">
	    				<option value=""></option>
    					<option value="cEntrant">Contact Entrant</option>
    					<option value="cDigital">Contact Digital</option>
    					<option value="cEvenement">Contact Evénements</option>  
    					<option value="cPersonnel">Contact Personnel</option>
    					<option value="cProjet">Contact identifié sur un autre projet</option>  
				</select></td></tr>';

			// Canal de connaissance du projet
				print '<tr id="canalconnaissanceprojet1" style="display:none;"><td>'.fieldLabel('Canal de connaissance du projet','ccp').'</td>';
	   			print '<td id="contactentrant" class="hidden"><select  id="ccpce" onchange="showtccp(this);">
	   			 		<option value=""></option>
    					<option value="Passage">Passage</option>
    					<option value="AppelEntrant">Appel entrant</option>
    					<option value="Parrainage">Parrainage</option>  
    					<option value="Mandataire">Mandataire</option>
				</select></td>';

				// Moyen de connaissance
				// tccp Contact Entrant
				// Origine Publicitaire
				print '<td class="hidden" id="labeloriginepub">'.fieldLabel('Origine publicitaire','tccp').'</td>';
	   			print '<td class="hidden" id="originepub"><input type="text" name="" id="originePub" class="maxwidth100onsmartphone quatrevingtpercent" ></td>';
	   			// Parainnage
	   			print '<td class="hidden" id="labelparrain">'.fieldLabel('Nom du parrain','tccp').'</td>';
	   			print '<td class="hidden" id="parrain"><input type="text" name="" id="parrainages" class="maxwidth100onsmartphone quatrevingtpercent" value="'.$object->tccp.'"></td>';
	   			// Mandataire
	   			print '<td class="hidden" id="labelmandataire">'.fieldLabel('Nom du mandataire','tccp').'</td>';
	   			print '<td class="hidden" id="mandataire"><input type="text"name="" id="mandataires" class="maxwidth100onsmartphone quatrevingtpercent" value="'.$object->tccp.'"></td></tr>';


	   			// Contact Digital
	   			print '<tr id="canalconnaissanceprojet2" style="display:none;"><td>'.fieldLabel('Canal de connaissance du projet','ccp').'</td>';
	   			print '<td id="contactdigital" class="hidden"><select id="ccpcd" onchange="showtccp(this);">
	   			 		<option value=""></option>
    					<option value="RS">Réseaux sociaux</option>
    					<option value="SI">Site Internet </option>
    					<option value="Media">Media</option>  
    					<option value="Autre">Autre</option>
				</select></td></tr>';

				// Contact Evénements
				print '<tr style="display:none;" id="labelnomevenement"><td >'.fieldLabel('Nom de l’événement  ','tccp').'</td>';
	   			print '<td class="hidden" id="nomevenement"><input type="text" name="" id="nomevenements" class="maxwidth100onsmartphone quatrevingtpercent" value="'.$object->tccp.'"></td></tr>';

	   			// Contact identifié sur un autre projet
				print '<tr style="display:none;" id="labelnomprojet"><td >'.fieldLabel('Nom du projet   ','tccp').'</td>';
	   			print '<td class="hidden" id="nomprojet"><input type="text" name="" id="nomprojets" class="maxwidth100onsmartphone quatrevingtpercent" value="'.$object->tccp.'"></td></tr>';
        ...
        while ($i <= 6)
        {
            ...
            if ($idprof!='-')
            {
                ...
                if (($j % 2) == 0) print '<tr style="display:none!important;">';
        ...
        // Vat is used
        print '<tr style="display:none!important;"><td>'.fieldLabel('VATIsUsed','assujtva_value').'</td>';
        print '<td>';
        ...
        {
            print '<tr style="display:none!important;"><td>'.$langs->transcountry("LocalTax1IsUsed",$mysoc->country_code).'</td><td>';
        ...
        {
            print '<tr style="display:none!important;"><td>'.$langs->transcountry("LocalTax1IsUsed",$mysoc->country_code).'</td><td colspan="3">';
            print $form->selectyesno('localtax1assuj_value',(isset($conf->global->THIRDPARTY_DEFAULT_USELOCALTAX1)?$conf->global->THIRDPARTY_DEFAULT_USELOCALTAX1:0),1);
            print '</td></tr>';
        }
        elseif($mysoc->localtax2_assuj=="1")
        {
            print '<tr style="display:none!important;"><td>'.$langs->transcountry("LocalTax2IsUsed",$mysoc->country_code).'</td><td colspan="3">';
            print $form->selectyesno('localtax2assuj_value',(isset($conf->global->THIRDPARTY_DEFAULT_USELOCALTAX2)?$conf->global->THIRDPARTY_DEFAULT_USELOCALTAX2:0),1);
            print '</td></tr>';
        }

        // Type - Size
        print '<tr style="display:none!important;"><td>'.fieldLabel('ThirdPartyType','typent_id').'</td><td class="maxwidthonsmartphone">'."\n";
        ...
        // Legal Form
        print '<tr style="display:none!important;"><td>'.fieldLabel('JuridicalStatus','forme_juridique_code').'</td>';
        ...
        // Capital
        print '<tr style="display:none!important;"><td>'.fieldLabel('Capital','capital').'</td>';
	    print '<td colspan="3"><input type="text" name="capital" id="capital" size="10" value="'.$object->capital.'"> ';
        print '<span class="hideonsmartphone">'.$langs->trans("Currency".$conf->currency).'</span></td></tr>';

        if (! empty($conf->global->MAIN_MULTILANGS))
        {
            print '<tr style="display:none!important;"><td>'.fieldLabel('DefaultLang','default_lang').'</td><td colspan="3" class="maxwidthonsmartphone">'."\n";
		...
		if (!empty($conf->incoterm->enabled))
		{
			print '<tr style="display:none!important;">';
		...
		if (! empty($conf->categorie->enabled)  && ! empty($user->rights->categorie->lire))
		{
			...
			print '<tr class="visibleifcustomer" style="display:none!important;"><td class="toptd">' . fieldLabel('CustomersProspectsCategoriesShort', 'custcats') . '</td><td colspan="3">';
			...
			print '<tr class="visibleifsupplier"><td class="toptd" style="display:none!important;">' . fieldLabel('SuppliersCategoriesShort', 'suppcats') . '</td><td colspan="3">';
    ...
    if ($isCommercial != 1){
    	...
    	<script type="text/javascript">
    	$(document).ready(function(){
        $("#formsoc1 :input").prop("readonly", true);
        $("#commercial").prop("readonly", false); 
        ...
        if ($socid)
        {
            ...
            print '<form enctype="multipart/form-data" action="'.$_SERVER["PHP_SELF"].'?socid='.$object->id.'" method="post" name="formsoc" id="formsoc1">';
            ...
            if (! empty($conf->barcode->enabled))
            {
                print '<tr style="display:none!important;"><td class="tdtop">'.fieldLabel('Gencod','barcode').'</td>';
	            print '<td colspan="3"><input type="text" name="barcode" id="barcode" value="'.$object->barcode.'">';
                print '</td></tr>';
            }

            // Status
            print '<tr style="display:none!important;"><td>'.fieldLabel('Status','status').'</td><td colspan="3">';
	        ...
	        if (! empty($conf->socialnetworks->enabled))
	        {
	        	...
	        	if (! empty($conf->global->SOCIALNETWORKS_SKYPE))
	        	{
	        		print '<tr style="display:none!important;"><td>'.fieldLabel('Skype','skype').'</td>';
	        		print '<td colspan="3"><input type="text" name="skype" id="skype" value="'.$object->skype.'"></td></tr>';
	        	}
	        	// Twitter
	        	if (! empty($conf->global->SOCIALNETWORKS_TWITTER))
	        	{
	        		print '<tr style="display:none!important;"><td>'.fieldLabel('Twitter','twitter').'</td>';
	        		print '<td colspan="3"><input type="text" name="twitter" id="twitter" value="'.$object->twitter.'"></td></tr>';
	        	}
	        	// Facebook
	        	if (! empty($conf->global->SOCIALNETWORKS_FACEBOOK))
	        	{
	        		print '<tr style="display:none!important;"><td>'.fieldLabel('Facebook','facebook').'</td>';
            ...
            // Phone / Fax
            print '<tr><td>'.fieldLabel('Phone','phone').'</td>';
	        print '<td><input type="text" name="pho  ne" id="phone" class="maxwidth100onsmartphone quatrevingtpercent" value="'.$object->phone.'"></td>';
            print '<td style="display:none!important;">'.fieldLabel('Fax','fax').'</td>';
	        print '<td style="display:none!important;"><input type="text" name="fax" id="fax" class="maxwidth100onsmartphone quatrevingtpercent" value="'.$object->fax.'"></td></tr>';
	    ...
	    print '<td><select name="origin" id="source" onchange="showccp(this);">
            ...
            while ($i <= 6)
            {
                ...
                if ($idprof!='-')
                {
	                ...
	                if (($j % 2) == 0) print '<tr style="display:none!important;"s>';
            ...
            // VAT is used
            print '<tr style="display:none!important;"><td>'.fieldLabel('VATIsUsed','assujtva_value').'</td><td colspan="3">';
            ...
            if($mysoc->localtax1_assuj=="1" && $mysoc->localtax2_assuj=="1")
            {
                print '<tr style="display:none!important;"><td>'.fieldLabel($langs->transcountry("LocalTax1IsUsed",$mysoc->country_code),'localtax1assuj_value').'</td><td>';
            ...
            {
                print '<tr style="display:none!important;"><td>'.fieldLabel($langs->transcountry("LocalTax1IsUsed",$mysoc->country_code),'localtax1assuj_value').'</td><td colspan="3">';
                print $form->selectyesno('localtax1assuj_value',$object->localtax1_assuj,1);
                if(! isOnlyOneLocalTax(1))
                {
                    print '<span class="cblt1">     '.$langs->transcountry("Type",$mysoc->country_code).': ';
            ...
            {
                print '<tr style="display:none!important;"><td>'.fieldLabel($langs->transcountry("LocalTax2IsUsed",$mysoc->country_code),'localtax2assuj_value').'</td><td colspan="3">';
                print $form->selectyesno('localtax2assuj_value',$object->localtax2_assuj,1);
                if(! isOnlyOneLocalTax(2))
                {
                    print '<span class="cblt2">     '.$langs->transcountry("Type",$mysoc->country_code).': ';
            ...
            // VAT Code
            print '<tr style="display:none!important;"><td>'.fieldLabel('VATIntra','intra_vat').'</td>';
            print '<td colspan="3">';
            $s ='<input type="text" class="flat maxwidthonsmartphone" name="tva_intra" id="intra_vat" maxlength="20" value="'.$object->tva_intra.'">';
            ...
            // Juridical type
            print '<tr style="display:none!important;"><td>'.fieldLabel('JuridicalStatus','forme_juridique_code').'</td><td class="maxwidthonsmartphone" colspan="3">';
            print $formcompany->select_juridicalstatus($object->forme_juridique_code, $object->country_code, '', 'forme_juridique_code');
            print '</td></tr>';
            ...
            if (! empty($conf->global->MAIN_MULTILANGS))
            {
                print '<tr style="display:none!important;"><td>'.fieldLabel('DefaultLang','default_lang').'</td><td colspan="3">'."\n";
			...
			if (! empty($conf->categorie->enabled)  && ! empty($user->rights->categorie->lire))
			{
				...
				// Supplier
				print '<tr class="visibleifsupplier" style="display:none!important;"><td>' . fieldLabel('SuppliersCategoriesShort', 'suppcats') . '</td>';
				print '<td colspan="3">';
            ...
            if (!empty($conf->syncsupplierwebservices->enabled)) {
                print '<tr style="display:none!important;"><td>'.fieldLabel('WebServiceURL','webservices_url').'</td>';
            ...
            print '<input type="submit" class="button" name="save" id="save" value="'.$langs->trans("Save").'">';
            print ' &nbsp; &nbsp; ';
    ...
    else
    {
        ...
        $linkback = '<a href="'.DOL_URL_ROOT.'/custom/gestiondescommerciaux/list.php?restore_lastsearch_values=1">'.$langs->trans("BackToList").'</a>';
    	...
    	{
    		print '<tr style="display:none!important;"><td>'.$langs->trans('Supplier').'</td><td>';
        ...
        if (! empty($conf->global->SOCIETE_USEPREFIX))  // Old not used prefix field
        {
            print '<tr style="display:none!important;"><td>'.$langs->trans('Prefix').'</td><td>'.$object->prefix_comm.'</td>';
        ...
        {
            print '<tr style="display:none!important;"><td>';
        ...
        if ($object->fournisseur || $mysoc->country_code=='ES')
        {
        	if($mysoc->localtax1_assuj=="1" && $mysoc->localtax2_assuj=="1")
			{
			    ...
			    print '</td></tr><tr style="display:none!important;"><td>'.$langs->transcountry("LocalTax2IsUsed",$mysoc->country_code).'</td><td>';
			    ...
			    if($object->localtax1_assuj=="1" && (! isOnlyOneLocalTax(1)))
			    {
			        ...
			        print '<input type="hidden" name="token" value="'.$_SESSION['newtoken'].'">';
			        print '<tr style="display:none!important;"><td>'.$langs->transcountry("TypeLocaltax1", $mysoc->country_code).' <a href="'.$_SERVER["PHP_SELF"].'?action=editRE&amp;socid='.$object->id.'">'.img_edit($langs->transnoentitiesnoconv('Edit'),1).'</td>';
			        if($action == 'editRE')
			        {
			            print '<td align="left">';
			            $formcompany->select_localtax(1,$object->localtax1_value, "lt1");
			    ...
			    if($object->localtax2_assuj=="1" && (! isOnlyOneLocalTax(2)))
			    {
			        ...
			        print '<input type="hidden" name="token" value="'.$_SESSION['newtoken'].'">';
			        print '<tr style="display:none!important;"><td>'.$langs->transcountry("TypeLocaltax2", $mysoc->country_code).'<a href="'.$_SERVER["PHP_SELF"].'?action=editIRPF&amp;socid='.$object->id.'">'.img_edit($langs->transnoentitiesnoconv('Edit'),1).'</td>';
			        if($action == 'editIRPF'){
			            print '<td align="left">';
			            $formcompany->select_localtax(2,$object->localtax2_value, "lt2");
			            print '<input type="submit" class="button" value="'.$langs->trans("Modify").'"></td>';
			...
			elseif($mysoc->localtax2_assuj=="1" && $mysoc->localtax1_assuj!="1")
			{
			    print '<tr style="display:none!important;"><td>'.$langs->transcountry("LocalTax2IsUsed",$mysoc->country_code).'</td><td>';
			    print yn($object->localtax2_assuj);
			    print '</td></tr>';
			    if($object->localtax2_assuj=="1" && (! isOnlyOneLocalTax(2)))
			    {
        ...
        print '<tr style="display:none!important;"><td>'.$langs->trans("ThirdPartyType").'</td><td>'.$object->typent.'</td>';
        print '<tr style="display:none!important;"><td>'.$langs->trans("Staff").'</td><td>'.$object->effectif.'</td></tr>';
		...
		if (! empty($conf->categorie->enabled)  && ! empty($user->rights->categorie->lire))
		{
			...
			// Supplier
			if ($object->fournisseur) {
				print '<tr style="display:none!important;"><td>' . $langs->trans("SuppliersCategoriesShort") . '</td>';
				print '<td>';
        ...
        // Legal
        print '<tr style="display:none!important;"><td class="titlefield">'.$langs->trans('JuridicalStatus').'</td><td>'.$object->forme_juridique.'</td></tr>';

        // Capital
        print '<tr style="display:none!important;"><td>'.$langs->trans('Capital').'</td><td>';
        ...
        {
            require_once DOL_DOCUMENT_ROOT.'/core/lib/functions2.lib.php';
            print '<tr style="display:none!important;"><td>'.$langs->trans("DefaultLang").'</td><td>';
        ...
        if (!empty($conf->incoterm->enabled))
        {
        	...
        	if ($user->rights->societe->creer) print '<a href="'.DOL_URL_ROOT.'/custom/gestiondescommerciaux/card.php?socid='.$object->id.'&action=editincoterm">'.img_edit('',1).'</a>';
        	else print '&nbsp;';
        	print '</td></tr></table>';
        	print '</td>';
        	print '<td colspan="3">';
        ...
        {
            $langs->load("members");
            print '<tr style="display:none!important;"><td>'.$langs->trans("LinkedToDolibarrMember").'</td>';
            print '<td colspan="3">';
            $adh=new Adherent($db);
        ...
        // Webservices url/key
        if (!empty($conf->syncsupplierwebservices->enabled)) {
            print '<tr style="display:none!important;"><td>'.$langs->trans("WebServiceURL").'</td><td>'.dol_print_url($object->webservices_url).'</td>';
