...
require_once DOL_DOCUMENT_ROOT.'/custom/gestiondescommerciaux/class/rights.class.php';
...
$search_origin=trim(GETPOST('search_origin'));
$search_ccp=trim(GETPOST('search_ccp'));
$search_tccp=trim(GETPOST('search_tccp'));

$search_dateFirstCall=trim(GETPOST('search_dateFirstCall'));
$search_dateSecondCall=trim(GETPOST('search_dateSecondCall'));
$search_dateThirdCall=trim(GETPOST('search_dateThirdCall'));
$search_issueFirstCall=trim(GETPOST('search_issueFirstCall'));
$search_issueSecondCall=trim(GETPOST('search_issueSecondCall'));
$search_issueThirdCall=trim(GETPOST('search_issueThirdCall'));
$search_dateFirstMeet=trim(GETPOST('search_dateFirstMeet'));
$search_dateSecondMeet=trim(GETPOST('search_dateSecondMeet'));
$search_result=trim(GETPOST('search_result'));
...
$Rightcommercial = new RightsGestionDesCommerciaux($db);

$isnotCommercial = $Rightcommercial->checkIfCommercial($user->id);
...
$fieldstosearchall = array(
	...
	's.phone'=>"Phone",
	's.origin'=>"Source",
	's.ccp'=>"Canal de connaissance du projet",
	's.tccp'=>"Moyen de connaissance",
	's.dateFirstCall'=>"Date 1st appel",
	's.dateSecondCall'=>"Date 2nd appel",
	's.dateThirdCall'=>"Date 3rd appel",
	's.issueFirstCall'=>"Issue 1st appel",
	's.issueSecondCall'=>"Issue 2nd appel",
	's.issueThirdCall'=>"Issue 3rd appel",
	's.dateFirstMeet'=>"Date R1",
	's.dateSecondMeet'=>"Date R2",
	's.result'=>"Statut de la Relation",
	's.fax'=>"Fax",
);
...
$arrayfields=array(
	...
	's.origin'=>array('label'=>"Source", 'checked'=>1),
	's.ccp'=>array('label'=>"Canal de connaissance du projet", 'checked'=>1),
	's.tccp'=>array('label'=>"Moyen de connaissance", 'checked'=>1),
	's.dateFirstCall'=>array('label'=>"Date 1st appel", 'checked'=>1),
	's.dateSecondCall'=>array('label'=>"Date 2nd appel", 'checked'=>1),
	's.dateThirdCall'=>array('label'=>"Date 3rd appel", 'checked'=>1),
	's.issueFirstCall'=>array('label'=>"Issue 1st appel", 'checked'=>1),
	's.issueSecondCall'=>array('label'=>"Issue 2nd appel", 'checked'=>1),
	's.issueThirdCall'=>array('label'=>"Issue 3rd appel", 'checked'=>1),
	's.dateFirstMeet'=>array('label'=>"Date R1", 'checked'=>1),
	's.dateSecondMeet'=>array('label'=>"Date R2", 'checked'=>1),
	's.result'=>array('label'=>"Statut de la Relation", 'checked'=>1),
...
if (empty($reshook))
{
	...
	if (GETPOST('button_removefilter_x','alpha') || GETPOST('button_removefilter.x','alpha') || GETPOST('button_removefilter','alpha')) // All tests are required to be compatible with all browsers
	{
		...
		$search_ccp='';
		$search_tccp='';
		$search_dateFirstCall='';
		$search_dateSecondCall='';
		$search_dateThirdCall='';
		$search_issueFirstCall='';
		$search_issueSecondCall='';
		$search_issueThirdCall='';
		$search_dateFirstMeet='';
		$search_dateSecondMeet='';
		$search_result='';
	...
	if ($action == 'setdate')
	{
		...
		$result=$db->query($sql);
		if(! $result ) {
               die('Could not update data: ' . $db->error());
            }
...
$sql.= " s.email, s.phone, s.fax, s.origin, s.ccp, s.tccp, s.dateFirstCall, s.dateSecondCall, s.dateThirdCall, s.issueFirstCall, s.issueSecondCall, s.issueThirdCall, s.dateFirstMeet, s.dateSecondMeet, s.result, s.url, s.siren as idprof1, s.siret as idprof2, s.ape as idprof3, s.idprof4 as idprof4, s.idprof5 as idprof5, s.idprof6 as idprof6, s.tva_intra, s.fk_pays,";
...
if (strlen($search_origin)) $sql.= natural_search("s.origin", $search_origin);
if (strlen($search_ccp)) $sql.= natural_search("s.ccp", $search_ccp);
if (strlen($search_tccp)) $sql.= natural_search("s.tccp", $search_tccp);
if (strlen($search_dateFirstCall)) $sql.= natural_search("s.dateFirstCall", $search_dateFirstCall);
if (strlen($search_dateSecondCall)) $sql.= natural_search("s.dateSecondCall", $search_dateSecondCall);
if (strlen($search_dateThirdCall)) $sql.= natural_search("s.dateThirdCall", $search_dateThirdCall);
if (strlen($search_issueFirstCall)) $sql.= natural_search("s.issueFirstCall", $search_issueFirstCall);
if (strlen($search_issueSecondCall)) $sql.= natural_search("s.issueSecondCall", $search_issueSecondCall);
if (strlen($search_issueThirdCall)) $sql.= natural_search("s.issueThirdCall", $search_issueThirdCall);
if (strlen($search_dateFirstMeet)) $sql.= natural_search("s.dateFirstMeet", $search_dateFirstMeet);
if (strlen($search_dateSecondMeet)) $sql.= natural_search("s.dateSecondMeet", $search_dateSecondMeet);
if (strlen($search_result)) $sql.= natural_search("s.result", $search_result);
...
if ($search_origin != '')   $param.= "&search_origin=".urlencode($search_origin);
if ($search_ccp != '')   $param.= "&search_ccp=".urlencode($search_ccp);
if ($search_tccp != '')   $param.= "&search_tccp=".urlencode($search_tccp);

if ($search_dateFirstCall != '')   $param.= "&search_dateFirstCall=".urlencode($search_dateFirstCall);
if ($search_dateSecondCall != '')   $param.= "&search_dateSecondCall=".urlencode($search_dateSecondCall);
if ($search_dateThirdCall != '')   $param.= "&search_dateThirdCall=".urlencode($search_dateThirdCall);
if ($search_issueFirstCall != '')   $param.= "&search_issueFirstCall=".urlencode($search_issueFirstCall);
if ($search_issueSecondCall != '')   $param.= "&search_issueSecondCall=".urlencode($search_issueSecondCall);
if ($search_issueThirdCall != '')   $param.= "&search_issueThirdCall=".urlencode($search_issueThirdCall);
if ($search_dateFirstMeet != '')   $param.= "&search_dateFirstMeet=".urlencode($search_dateFirstMeet);
if ($search_dateSecondMeet != '')   $param.= "&search_dateSecondMeet=".urlencode($search_dateSecondMeet);
if ($search_result != '')   $param.= "&search_result=".urlencode($search_result);
...
if ($user->rights->societe->creer)
{
	...
	$newcardbutton = '<a class="butActionNew" href="'.DOL_URL_ROOT.'/custom/gestiondescommerciaux/card.php?action=create'.$typefilter.'"><span class="valignmiddle">'.$langs->trans($label).'</span>';
...
if (! empty($arrayfields['s.origin']['checked']))
{
	//origin
	print '<td class="liste_titre">';
	print '<input class="flat searchstring maxwidth50" type="text" name="search_origin" value="'.dol_escape_htmltag($search_origin).'">';
	print '</td>';
}
if (! empty($arrayfields['s.ccp']['checked']))
{
	// ccp
	print '<td class="liste_titre">';
	print '<input class="flat searchstring maxwidth50" type="text" name="search_ccp" value="'.dol_escape_htmltag($search_ccp).'">';
	print '</td>';
}
if (! empty($arrayfields['s.tccp']['checked']))
{
	// tccp
	print '<td class="liste_titre">';
	print '<input class="flat searchstring maxwidth50" type="text" name="search_tccp" value="'.dol_escape_htmltag($search_tccp).'">';
	print '</td>';
}

if (! empty($arrayfields['s.dateFirstCall']['checked']))
{
	// dateFirstCall
	print '<td class="liste_titre">';
	print '<input class="flat search maxwidth50" type="text" name="search_dateFirstCall" value="'.dol_escape_htmltag($search_dateFirstCall).'">';
	print '</td>';
}
if (! empty($arrayfields['s.dateSecondCall']['checked']))
{
	// dateSecondCall
	print '<td class="liste_titre">';
	print '<input class="flat searchstring maxwidth50" type="text" name="search_dateSecondCall" value="'.dol_escape_htmltag($search_dateSecondCall).'">';
	print '</td>';
}
if (! empty($arrayfields['s.dateThirdCall']['checked']))
{
	// dateThirdCall
	print '<td class="liste_titre">';
	print '<input class="flat searchstring maxwidth50" type="text" name="search_dateThirdCall" value="'.dol_escape_htmltag($search_dateThirdCall).'">';
	print '</td>';
}
if (! empty($arrayfields['s.issueThirdCall']['checked']))
{
	// issueThirdCall
	print '<td class="liste_titre">';
	print '<input class="flat searchstring maxwidth50" type="text" name="search_issueThirdCall" value="'.dol_escape_htmltag($search_issueThirdCall).'">';
	print '</td>';
}
if (! empty($arrayfields['s.issueThirdCall']['checked']))
{
...
if (! empty($arrayfields['s.dateFirstMeet']['checked']))
{
	// dateFirstMeet
	print '<td class="liste_titre">';
	print '<input class="flat searchstring maxwidth50" type="text" name="search_dateFirstMeet" value="'.dol_escape_htmltag($search_dateFirstMeet).'">';
	print '</td>';
}
if (! empty($arrayfields['s.dateSecondMeet']['checked']))
{
	// dateSecondMeet
	print '<td class="liste_titre">';
	print '<input class="flat searchstring maxwidth50" type="text" name="search_dateSecondMeet" value="'.dol_escape_htmltag($search_dateSecondMeet).'">';
	print '</td>';
}
if (! empty($arrayfields['s.result']['checked']))
{
	// result
	print '<td class="liste_titre">';
	print '<input class="flat searchstring maxwidth50" type="text" name="search_result" value="'.dol_escape_htmltag($search_result).'">';
	print '</td>';
}
...
if (! empty($arrayfields['s.origin']['checked']))          print_liste_field_titre($arrayfields['s.origin']['label'],$_SERVER["PHP_SELF"],"s.origin","",$param,'',$sortfield,$sortorder);
if (! empty($arrayfields['s.ccp']['checked']))          print_liste_field_titre($arrayfields['s.ccp']['label'],$_SERVER["PHP_SELF"],"s.ccp","",$param,'',$sortfield,$sortorder);
if (! empty($arrayfields['s.tccp']['checked']))          print_liste_field_titre($arrayfields['s.tccp']['label'],$_SERVER["PHP_SELF"],"s.tccp","",$param,'',$sortfield,$sortorder);

if (! empty($arrayfields['s.dateFirstCall']['checked']))          print_liste_field_titre($arrayfields['s.dateFirstCall']['label'],$_SERVER["PHP_SELF"],"s.dateFirstCall","",$param,'',$sortfield,$sortorder);
if (! empty($arrayfields['s.dateSecondCall']['checked']))          print_liste_field_titre($arrayfields['s.dateSecondCall']['label'],$_SERVER["PHP_SELF"],"s.dateSecondCall","",$param,'',$sortfield,$sortorder);
if (! empty($arrayfields['s.dateThirdCall']['checked']))          print_liste_field_titre($arrayfields['s.dateThirdCall']['label'],$_SERVER["PHP_SELF"],"s.dateThirdCall","",$param,'',$sortfield,$sortorder);
if (! empty($arrayfields['s.issueFirstCall']['checked']))          print_liste_field_titre($arrayfields['s.issueFirstCall']['label'],$_SERVER["PHP_SELF"],"s.issueFirstCall","",$param,'',$sortfield,$sortorder);
if (! empty($arrayfields['s.issueSecondCall']['checked']))          print_liste_field_titre($arrayfields['s.issueSecondCall']['label'],$_SERVER["PHP_SELF"],"s.issueSecondCall","",$param,'',$sortfield,$sortorder);
if (! empty($arrayfields['s.issueThirdCall']['checked']))          print_liste_field_titre($arrayfields['s.issueThirdCall']['label'],$_SERVER["PHP_SELF"],"s.issueThirdCall","",$param,'',$sortfield,$sortorder);

if (! empty($arrayfields['s.dateFirstMeet']['checked']))          print_liste_field_titre($arrayfields['s.dateFirstMeet']['label'],$_SERVER["PHP_SELF"],"s.dateFirstMeet","",$param,'',$sortfield,$sortorder);
if (! empty($arrayfields['s.dateSecondMeet']['checked']))          print_liste_field_titre($arrayfields['s.dateSecondMeet']['label'],$_SERVER["PHP_SELF"],"s.dateSecondMeet","",$param,'',$sortfield,$sortorder);
if (! empty($arrayfields['s.result']['checked']))          print_liste_field_titre($arrayfields['s.result']['label'],$_SERVER["PHP_SELF"],"s.result","",$param,'',$sortfield,$sortorder);
...
while ($i < min($num, $limit))
{
	...
	{
		switch ($obj->origin){
		case "cEntrant": $sourcecontact= "Contact Entrant";
							break;
		case "cPersonnel": $sourcecontact= "Contact Personnel";
							break;
		case "cDigital": $sourcecontact= "Contact Digital";
							break;
		case "cEvenement": $sourcecontact= "Contact Événements";
							break;
		case "cProjet": $sourcecontact= "Contact identifié sur un autre projet";
							break;
		case "": $sourcecontact= "";
							break;
	}
		print "<td>".$sourcecontact."</td>\n";
		if (! $i) $totalarray['nbfield']++;
	}
	if (! empty($arrayfields['s.ccp']['checked']))
	{
		switch ($obj->ccp){
		case "RS": $canal= "Réseaux sociaux";
							break;
		case "SI": $canal= "Site Internet";
							break;
		case "Media": $canal= "Media";
							break;
		case "Autre": $canal= "Autre";
							break;
		case "Passage": $canal= "Passage";
							break;
		case "AppelEntrant": $canal= "Appel entrant";
							break;
		case "Parrainage": $canal= "Parrainage";
							break;
		case "Mandataire": $canal= "Mandataire";
							break;
		case "": $canal= "";
							break;
	}
	...
	if (! empty($arrayfields['s.tccp']['checked']))
	{
		print "<td>".$obj->tccp."</td>\n";
		if (! $i) $totalarray['nbfield']++;
	}
	
	if (! empty($arrayfields['s.dateFirstCall']['checked']))
	{
		if(! empty($obj->dateFirstCall)){
		print "<td>".$obj->dateFirstCall."</td>\n";
		}
		else{
		print "<td><input name='dateFirstCall".$i."' value='".$obj->dateFirstCall."' type=date></td>\n";
		}
		if (! $i) $totalarray['nbfield']++;
	}
	 ...
	 print "\n".'<script type="text/javascript">';
            print '
					function handler(e) {
						...
						$column = e.target.id;
						$.ajax({
							url: "list.php?action=setdate",
							type: "POST",
							data:{date:$date, rowid:$rowid, column:$column},
							datatype:"html",
							});
	...
	if (! empty($arrayfields['s.dateSecondCall']['checked']))
	{	
		if(! empty($obj->dateSecondCall)){
			if ($isnotCommercial != 1){
				print "<td>".$obj->dateSecondCall."</td>\n";
			}
			else {
				print "<td><input name='".$obj->rowid."' id='dateSecondCall' value='".$obj->dateSecondCall."' type=date onchange='handler(event);'></td>\n";
			}
		}
		else{print "<td><input name='".$obj->rowid."' id='dateSecondCall' value='".$obj->dateSecondCall."' type=date onchange='handler(event);'></td>\n";}
		if (! $i) $totalarray['nbfield']++;
	}
	if (! empty($arrayfields['s.dateThirdCall']['checked']))
	{	
		if(! empty($obj->dateThirdCall)){
			if ($isnotCommercial != 1){
				print "<td>".$obj->dateThirdCall."</td>\n";
			}
			else {
				print "<td><input name='".$obj->rowid."' id='dateThirdCall' value='".$obj->dateThirdCall."' type=date onchange='handler(event);'></td>\n";
			}
		}
		else{print "<td><input name='".$obj->rowid."' id='dateThirdCall' value='".$obj->dateThirdCall."' type=date onchange='handler(event);'></td>\n";}
		if (! $i) $totalarray['nbfield']++;
	}
	if (! empty($arrayfields['s.issueFirstCall']['checked']))
	{	
		if(! empty($obj->issueFirstCall)){
		switch ($obj->issueFirstCall){
		case "Rappeler": $issuecall= "A rappeler";
							break;
		case "Fixe": $issuecall= "Rendez-vous fixé";
							break;
		case "PasInteresse": $issuecall= "Pas intéressé";
							break;
		case "NeRepondpas": $issuecall= "Ne répond pas";
							break;
		case "Occupe": $issuecall= "Occupé";
							break;
		case "TelEteint": $issuecall= "Tél éteint";
							break;
		case "": $issuecall= "";
							break;
		}
			if ($isnotCommercial != 1){
			print "<td>".$issuecall."</td>\n";
			}
			else {
			print "<td><select name='".$obj->rowid."' id='issueFirstCall' onchange='handler(event);'>
							<option value=''></option>
    						<option value='Rappeler' ".(($issuecall === 'A rappeler')?'selected="selected"':'').">A rappeler </option>
    						<option value='Fixe' ".(($issuecall === 'Rendez-vous fixé')?'selected="selected"':'').">Rendez-vous fixé </option>
    						<option value='PasInteresse' ".(($issuecall === 'Pas intéressé')?'selected="selected"':'').">Pas intéressé </option>  
    						<option value='NeRepondpas' ".(($issuecall === 'Ne répond pas')?'selected="selected"':'').">Ne répond pas </option>
    						<option value='Occupe' ".(($issuecall === 'Occupé')?'selected="selected"':'').">Occupé </option> 
    						<option value='TelEteint' ".(($issuecall === 'Tél éteint')?'selected="selected"':'').">Tél éteint  </option>  
						</select</td>\n";	
			}
		}
		else{print "<td><select name='".$obj->rowid."' id='issueFirstCall' onchange='handler(event);'>
							<option value=''></option>
    						<option value='Rappeler'>A rappeler </option>
    						<option value='Fixe'>Rendez-vous fixé </option>
    						<option value='PasInteresse'>Pas intéressé </option>  
    						<option value='NeRepondpas'>Ne répond pas </option>
    						<option value='Occupe'>Occupé </option> 
    						<option value='TelEteint'>Tél éteint  </option>  
						</select</td>\n";}
		if (! $i) $totalarray['nbfield']++;
	}
	if (! empty($arrayfields['s.issueSecondCall']['checked']))
	{	
		if(! empty($obj->issueSecondCall)){
		switch ($obj->issueSecondCall){
		case "Rappeler": $issuecall= "A rappeler";
							break;
		case "Fixe": $issuecall= "Rendez-vous fixé";
							break;
		...
		else{print "<td><select name='".$obj->rowid."' id='issueSecondCall' onchange='handler(event);'>
							<option value=''></option>
    						<option value='Rappeler'>A rappeler </option>
    						<option value='Fixe'>Rendez-vous fixé </option>
    						<option value='PasInteresse'>Pas intéressé </option>  
	...
	if (! empty($arrayfields['s.issueThirdCall']['checked']))
	{	
		if(! empty($obj->issueThirdCall)){
		switch ($obj->issueThirdCall){
		case "Rappeler": $issuecall= "A rappeler";
							break;
		case "Fixe": $issuecall= "Rendez-vous fixé";
							break;
		...
		else{print "<td><select name='".$obj->rowid."' id='issueThirdCall' onchange='handler(event);'>
							<option value=''></option>
    						<option value='Rappeler'>A rappeler </option>
    						<option value='Fixe'>Rendez-vous fixé </option>
    						<option value='PasInteresse'>Pas intéressé </option>  
	...
	if (! empty($arrayfields['s.dateFirstMeet']['checked']))
	{
		if(! empty($obj->dateFirstMeet)){
		if ($isnotCommercial != 1){
				print "<td>".$obj->dateFirstMeet."</td>\n";
			}
			else {
				print "<td><input name='".$obj->rowid."' id='dateFirstMeet' value='".$obj->dateFirstMeet."' type=date onchange='handler(event);'></td>\n";
			}
		}
		else{print "<td><input name='".$obj->rowid."' id='dateFirstMeet' value='".$obj->dateFirstMeet."' type=date onchange='handler(event);'></td>\n";}
		if (! $i) $totalarray['nbfield']++;
	}
	if (! empty($arrayfields['s.dateSecondMeet']['checked']))
	{
		if(! empty($obj->dateSecondMeet)){
		if ($isnotCommercial != 1){
				print "<td>".$obj->dateSecondMeet."</td>\n";
			}
			else {
				print "<td><input name='".$obj->rowid."' id='dateSecondMeet' value='".$obj->dateSecondMeet."' type=date onchange='handler(event);'></td>\n";
			}
		}
		else{print "<td><input name='".$obj->rowid."' id='dateSecondMeet' value='".$obj->dateSecondMeet."' type=date onchange='handler(event);'></td>\n";}
		if (! $i) $totalarray['nbfield']++;
	}
	if (! empty($arrayfields['s.result']['checked']))
	{	
		if(! empty($obj->result)){
		switch ($obj->result){
		case "RPOffreorale": $result= "Retour Positif - Offre Orale";
							break;
		case "RPResa": $result= "Retour Positif - Résa";
							break;
		case "RPCompromis": $result= "Retour Positif - Compromis / Promesse";
							break;
		case "RNNqualifie": $result= "Retour Négatif - Non Qualifié";
							break;
		case "RNInteresse": $result= "Retour Négatif - Intéressé LT < 6 mois";
							break;
		case "RNAutreprojet": $result= "Retour Négatif - Redirection vers autre projet";
							break;
		case "RNHesitant": $result= "Retour Négatif - Hésitant / Sceptique";
							break;
		case "RNInjoignabe": $result= "Retour Négatif - Injoignable";
							break;
		case "": $result= "";
							break;
		}
		if ($isnotCommercial != 1){
			...
			print "<td><select name='".$obj->rowid."' id='result' onchange='handler(event);'>
							<option value=''></option>
    						<option value='RPOffreorale'".(($result === 'Retour Positif - Offre Orale')?'selected="selected"':'').">Retour Positif - Offre Orale </option>
    						<option value='RPResa'".(($result === 'Retour Positif - Résa')?'selected="selected"':'').">Retour Positif - Résa </option>
    						<option value='RPCompromis'".(($result === 'Retour Positif - Compromis / Promesse')?'selected="selected"':'').">Retour Positif - Compromis / Promesse </option>  
    						<option value='RNNqualifie'".(($result === 'Retour Négatif - Non Qualifié')?'selected="selected"':'').">Retour Négatif - Non Qualifié </option>
    						<option value='RNInteresse'".(($result === 'Retour Négatif - Intéressé LT < 6 mois')?'selected="selected"':'').">Retour Négatif - Intéressé LT < 6 mois </option> 
    						<option value='RNAutreprojet'".(($result === 'Retour Négatif - Redirection vers autre projet')?'selected="selected"':'').">Retour Négatif - Redirection vers autre projet  </option> 
    						<option value='RNHesitant'".(($result === 'Retour Négatif - Hésitant / Sceptique')?'selected="selected"':'').">Retour Négatif - Hésitant / Sceptique </option> 
    						<option value='RNInjoignabe'".(($result === 'Retour Négatif - Injoignable')?'selected="selected"':'').">Retour Négatif - Injoignable </option>  
						</select</td>\n";
			}
		}
		else{print "<td><select name='".$obj->rowid."' id='result' onchange='handler(event);'>
							<option value=''></option>
    						<option value='RPOffreorale'>Retour Positif - Offre Orale </option>
    						<option value='RPResa'>Retour Positif - Résa </option>
    						<option value='RPCompromis'>Retour Positif - Compromis / Promesse </option>  
    						<option value='RNNqualifie'>Retour Négatif - Non Qualifié </option>
    						<option value='RNInteresse'>Retour Négatif - Intéressé LT < 6 mois </option> 
    						<option value='RNAutreprojet'>Retour Négatif - Redirection vers autre projet  </option> 
    						<option value='RNHesitant'>Retour Négatif - Hésitant / Sceptique </option> 
    						<option value='RNInjoignabe'>Retour Négatif - Injoignable </option>  
						</select</td>\n";}
		if (! $i) $totalarray['nbfield']++;
	}
