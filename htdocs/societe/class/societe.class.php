...
class Societe extends CommonObject
{
	...
	function create(User $user)
	{
		...
		$now=dol_now();
		$dateFirstCall=date('Y-m-d',$now+172800);
		...
		if ($result >= 0)
		{
			...
			$sql = "INSERT INTO ".MAIN_DB_PREFIX."societe (nom, name_alias, entity, datec, dateFirstCall, origin, ccp, tccp, fk_user_creat, canvas, status, ref_int, ref_ext, fk_stcomm, fk_incoterms, location_incoterms ,import_key, fk_multicurrency, multicurrency_code)";
			$sql.= " VALUES ('".$this->db->escape($this->name)."', '".$this->db->escape($this->name_alias)."', ".$this->db->escape($this->entity).", '".$this->db->idate($now)."', '".$this->db->idate($dateFirstCall)."', '".$this->db->escape($this->origin)."', '".$this->db->escape($this->ccp)."', '".$this->db->escape($this->tccp)."'";
	...
	function update($id, $user='', $call_trigger=1, $allowmodcodeclient=0, $allowmodcodefournisseur=0, $action='update', $nosyncmember=1)
	{
		...
		$this->ccp		= trim($this->ccp);
		$this->tccp		= trim($this->tccp);
		...
		if ($result >= 0)
		{
			...
			$sql .= ",ccp = ".(! empty($this->ccp)?"'".$this->db->escape($this->ccp)."'":"null");
			$sql .= ",tccp = ".(! empty($this->tccp)?"'".$this->db->escape($this->tccp)."'":"null");
	...
	function fetch($rowid, $ref='', $ref_ext='', $ref_int='', $idprof1='',$idprof2='',$idprof3='',$idprof4='',$idprof5='',$idprof6='', $email='', $ref_alias='')
	{
		...
		if ($resql)
		{
			...
			elseif ($num)   // $num = 1
			{
				...
				$this->dateFirstCall    = $this->db->jdate($obj->date_dateFirstCall);
	...
	function add_commercial(User $user, $commid)
	{
		...
		if ($this->id > 0 && $commid > 0)
		{
			...
			$sql.= " ( fk_soc, fk_user, timenow)";
			$sql.= " VALUES ('".$this->id."', '".$commid."', '".$this->db->idate($now)."')";
			if (! $this->db->query($sql) )
			{
	...
	function getNomUrl($withpicto=0, $option='', $maxlen=0, $notooltip=0, $save_lastsearch_value=-1)
	{
		...
		if (empty($linkstart))
		{
			...
			$linkstart = '<a href="'.DOL_URL_ROOT.'/custom/gestiondescommerciaux/card.php?socid='.$this->id;
		}
