...
if ($action == 'create')
{
	...
	// users
	print '<tr>';
        print '<td> Affecter un commercial	</td>';
		...
		$selected = (count(GETPOST('users', 'array')) > 0 ? GETPOST('users', 'array') : (GETPOST('users', 'int') > 0 ? array(GETPOST('users', 'int')) : (empty($user->rights->societe->client->voir)?array($user->id):array())));

        //print $form->multiselectarray('users', $userlist, $selected, null, null, null, null, "90%");
         print $form->multiselectarray('users', $userlist, GETPOST('users', 'array'), null, null, null, null, "90%");
...
else
{
	...
	if ($id > 0 || $ref)
	{
		...
		if ($action <> 'edit' && $action <> 're-edit')
		{
			...
			// Commercial
			print '<tr><td valign="middle"> Commercial</td><td colspan="3">';
			$c2 = new User($db);
			$users2 = $c2->containing($object->id);
			
			$arrayselected=array();
			if($users2){
			foreach($users2 as $user2) {
				$toprint[] = '<li class="select2-search-choice-dolibarr noborderoncategories" style="background: #aaa">'.$user2->lastname.' '.$user2->firstname.'</li>';
			}
	...
	print '<tr>';
        ...
        $c = new User($db);
		$users = $c->containing($object->id);

		$arrayselected=array();
		foreach($users as $user) {
			$arrayselected[] = $user->id;
		}


        print $form->multiselectarray('users', $userlist, $arrayselected, null, null, null, null, "90%");
        print '</td></tr>';
